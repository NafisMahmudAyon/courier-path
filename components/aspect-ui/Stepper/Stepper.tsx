...
const Stepper: React.FC<StepperProps> = ({ children, initialStep = 0, className = '' }) => {
...
const StepperItem: React.FC<StepperItemProps & { index?: number }> = ({
  ...
  return (
    <div className={cn('flex flex-col items-center', className)}>
      ...
      <p
        className={`mt-2 text-sm whitespace-nowrap ${active ? 'text-text font-medium' : 'text-text-muted'}`}
      >
        {label}
      </p>
